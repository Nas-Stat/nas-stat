...
Confirmed: current token scopes are `gist`, `read:org`, `repo`. GitHub refuses to push any file under `.github/workflows/` without the `workflow` scope.
...
The branch has 11 commits: 1 implementation + 10 quality report iterations. Do not let audit doc commits land on `main`. **Squash and merge only.**
...
- [ ] `git push -u origin issue-17-cicd-pipeline`
- [ ] Open PR → merge with **Squash and merge** (11 commits → 1)
- [ ] `gh issue close 17`
